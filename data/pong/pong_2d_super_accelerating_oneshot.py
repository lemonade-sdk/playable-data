# CREATE: pong with 2D paddles and rapidly accelerating ball

"""
Pong 2D Super Accelerating Game
...
rapidly with each paddle collision. The ball accelerates at a high rate, creating
extremely fast-paced gameplay that escalates quickly. Implementation uses pygame
with 2D movement controls, rapid speed scaling mechanics, collision detection,
and high-acceleration physics simulation.
"""
...
BALL_ACCELERATION = 1.0  # Double the normal acceleration rate
MAX_BALL_SPEED = 15  # Higher max speed for super acceleration
WINNING_SCORE = 5
...
GREEN = (0, 255, 65)
YELLOW = (255, 255, 0)
RED = (255, 100, 100)  # For extremely fast ball


class Game:
    def __init__(self):
        self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        pygame.display.set_caption("Pong Game - 2D Super Accelerating Ball")
    ...
    def accelerate_ball(self):
        # Increase ball speed when hit by paddle (double acceleration!)
    ...
    def update(self):
        if not self.game_over:
            ...
            if (
                ...
                self.accelerate_ball()  # Double acceleration on paddle hit
    ...
    def draw(self):
        ...
        # Draw ball (enhanced color progression for super acceleration)
        ball_color = WHITE
        if self.ball_speed > INITIAL_BALL_SPEED + 6:
            ball_color = RED  # Red when extremely fast
        elif self.ball_speed > INITIAL_BALL_SPEED + 3:
