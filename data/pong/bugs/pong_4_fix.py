...
BUG FIX: In the input method, the variable name used when checking key presses
doesn't match the variable name that was assigned. Look at what variable stores
the key state versus what variable is being accessed.
"""
...
class PongGame:
    ...
    def input(self):
        ...
        if not self.done:
            ...
            # Fixed: use correct variable name 'k' instead of 'keys'
            if k[pygame.K_w]:
                self.p1.up()
            if k[pygame.K_s]:
                self.p1.down()
