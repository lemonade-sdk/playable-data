# CREATE: flappy bird where the bird grows as I score

"""
Flappy Bird Growing Game

A side-scrolling game where the player controls a bird that increases in size as the score
increases, making navigation progressively more difficult. The bird falls due to gravity
and flaps upward when spacebar is pressed. Bird size scales with score, creating dynamic
difficulty progression. Implementation uses pygame with size scaling mechanics, collision
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Flappy Bird - Growing Remix")
    ...
    def update(self):
        if not self.game_over:
            ...
            if (
                self.bird.y - self.bird.size // 2 <= 0
                or self.bird.y + self.bird.size // 2 >= SCREEN_HEIGHT
            ):
                self.game_over = True
