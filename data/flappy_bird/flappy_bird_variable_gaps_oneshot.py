# CREATE: flappy bird with variable pipe gap sizes

"""
Flappy Bird Variable Gaps Game

A side-scrolling game where the player controls a bird that must navigate through pipe gaps
of varying sizes. The bird falls due to gravity and flaps upward when spacebar is pressed.
Gap sizes are randomized between pipes, creating unpredictable difficulty levels throughout
gameplay. Implementation uses pygame with random gap generation, collision detection, and
physics simulation.
"""
...
class Pipe:
    def __init__(self, x):
        self.x = x
        # Generate random gap size within bounds
        self.gap_size = random.randint(MIN_PIPE_GAP, MAX_PIPE_GAP)
        # Position gap randomly but ensure it fits on screen
        min_gap_y = 100
        max_gap_y = SCREEN_HEIGHT - 100 - self.gap_size
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Flappy Bird - Variable Gaps Remix")
