# CREATE: asteroids with rainbow colors

"""
Asteroids Rainbow Game

A space shooter where the player controls a ship to destroy asteroids that display
rainbow colors. Asteroids are rendered in different rainbow colors that change when
they split into smaller pieces. Implementation uses pygame with vector-based movement,
collision detection, and dynamic color assignment from a rainbow color palette.
"""
...
class Asteroid:
    ...
    def split(self):
        # Return smaller asteroids with new rainbow colors when this one is destroyed
        if self.size > 1:
            new_asteroids = []
            for _ in range(2):
                # Assign new random rainbow color to each split asteroid
                new_color = random.choice(RAINBOW_COLORS)
                new_asteroid = Asteroid(self.x, self.y, self.size - 1, new_color)
...
def main():
    ...
    pygame.display.set_caption("Asteroids Rainbow Remix")
            ...
            for bullet in bullets[:]:
                for asteroid in asteroids[:]:
                    if check_collision(
                        ...
                        # Split asteroid with new rainbow colors
