...
BUG FIX: The Asteroid class doesn't have a 'size' attribute - it has 'radius'.
Look at the collision_check call in the ship collision section and change the attribute name to match what's defined in the Asteroid class.
"""
...
def main():
    ...
    while running:
        ...
        if game_active:
            ...
            for asteroid in asteroids:
                if collision_check(
                    player.x, player.y, 12, asteroid.x, asteroid.y, asteroid.radius  # FIXED: Changed from asteroid.size to asteroid.radius
                ):
                    game_active = False
