# CREATE: asteroids with an enemy that chases me

"""
Asteroids Hunter Game

A space shooter where the player controls a ship to destroy asteroids while being
pursued by an AI enemy hunter. The hunter constantly moves toward the player ship
and can be knocked back by bullets but not destroyed. Implementation uses pygame
with vector-based movement, collision detection, enemy AI pathfinding, and
knockback physics for bullet-hunter interactions.
"""
...
class Enemy:
    def __init__(self, x, y):
        ...
        self.vel_x = 0
        self.vel_y = 0
        self.size = 24  # Doubled from 12 to 24
        self.max_speed = 1.5
    ...
    def update(self, target_x, target_y):
        ...
        if distance > 0:
            # Normalize direction and apply acceleration toward player
            dir_x = dx / distance
            dir_y = dy / distance

            self.vel_x += dir_x * self.acceleration
            self.vel_y += dir_y * self.acceleration
        ...
        # Apply slight velocity decay (for knockback recovery)
        self.vel_x *= self.knockback_decay
        self.vel_y *= self.knockback_decay

    def take_hit(self, bullet_vel_x, bullet_vel_y):
        # Apply knockback from bullet impact
        knockback_force = 3.0
        self.vel_x += bullet_vel_x * knockback_force * 0.1
        self.vel_y += bullet_vel_y * knockback_force * 0.1

    def draw(self, screen):
        # Draw enemy as red diamond
        points = []
        for angle_offset in [0, 90, 180, 270]:
            point_angle = math.radians(angle_offset)
            point_x = self.x + math.cos(point_angle) * self.size
            point_y = self.y + math.sin(point_angle) * self.size
            points.append((point_x, point_y))
...
def main():
    ...
    pygame.display.set_caption("Asteroids Hunter Remix")
    ...
    enemy_x = random.choice([100, SCREEN_WIDTH - 100])
    enemy_y = random.choice([100, SCREEN_HEIGHT - 100])
    ...
    while running:
        ...
        if not game_over:
            ...
            # Check bullet-enemy collisions (knockback only)
            for bullet in bullets[:]:
                if check_collision(bullet.x, bullet.y, 2, enemy.x, enemy.y, enemy.size):
                    enemy.take_hit(bullet.vel_x, bullet.vel_y)
                    bullets.remove(bullet)
                    score += 5  # Small bonus for hitting enemy
                    break
            ...
            if check_collision(ship.x, ship.y, ship.size, enemy.x, enemy.y, enemy.size):
                game_over = True
