...
BUG FIX: In the collision detection for player projectiles, the code checks for collisions
with game.spaceship instead of game.aliens. Change the second parameter to game.aliens.
"""
...
def main():
    ...
    while active:
        ...
        if not game.is_game_over:
            ...
            for proj in game.player_projectiles:
                hit_aliens = pygame.sprite.spritecollide(proj, game.aliens, True)  # Fixed: was game.spaceship
                if hit_aliens:
                    proj.kill()
        ...
        score_surface = text_font.render(f"Score: {game.player_score}", True, COLOR_PRIMARY)  # Fixed: was game.score
