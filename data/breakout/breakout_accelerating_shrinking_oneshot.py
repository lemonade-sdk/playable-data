# CREATE: breakout where the ball accelerates and the paddle shrinks

"""
Breakout Accelerating Shrinking Game

A brick-breaking game where the player controls a paddle that decreases in size when bricks
are destroyed, while the ball increases in speed with each paddle collision. The ball starts
at moderate speed and accelerates with each hit, while the paddle begins at standard size
and shrinks progressively. Implementation uses pygame with speed scaling mechanics, size
modification systems, collision detection, and physics simulation.
"""
...
class Paddle:
    ...
    def shrink(self):
        """Shrink paddle when brick is destroyed"""
        if self.width > MIN_PADDLE_WIDTH:
            old_width = self.width
            self.width = max(self.width - PADDLE_SHRINK_AMOUNT, MIN_PADDLE_WIDTH)
