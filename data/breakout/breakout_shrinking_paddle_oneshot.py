# CREATE: breakout where the paddle shrinks when bricks are destroyed

"""
Breakout Shrinking Paddle Game

A brick-breaking game where the player controls a paddle that decreases in size each
time a brick is destroyed, creating progressively more difficult gameplay. The paddle
gradually becomes smaller, requiring increased precision and skill as the game progresses.
Implementation uses pygame with paddle size modification mechanics, collision detection,
...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600
PADDLE_WIDTH = 120  # Start larger since it will shrink
PADDLE_HEIGHT = 15
PADDLE_SPEED = 8
BALL_SIZE = 15
BALL_SPEED = 5
...
BRICK_COLS = 10
BRICK_PADDING = 5
PADDLE_SHRINK_AMOUNT = 2  # Pixels to shrink per brick destroyed
MIN_PADDLE_WIDTH = 30  # Minimum paddle size
BLACK = (0, 0, 0)
GREEN = (0, 255, 65)
WHITE = (255, 255, 255)
RED = (255, 0, 0)
...
class Paddle:
    ...
    def shrink(self):
        if self.width > MIN_PADDLE_WIDTH:
            old_width = self.width
            self.width = max(MIN_PADDLE_WIDTH, self.width - PADDLE_SHRINK_AMOUNT)
            # Keep paddle centered when shrinking
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Breakout - Shrinking Paddle Remix")
        ...
        if not self.game_over:
            ...
            for brick in self.bricks:
                if not brick.destroyed and self.ball.get_rect().colliderect(
                    ...
                    self.ball.vel_y = -self.ball.vel_y
                    self.score += 10
                    self.paddle.shrink()  # Shrink paddle when brick is destroyed
                    break
