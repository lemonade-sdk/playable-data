# CREATE: breakout where the bricks move

"""
Breakout Moving Bricks Game

A brick-breaking game where the player controls a paddle to bounce a ball and destroy
bricks that slowly move side to side. Bricks oscillate horizontally, creating dynamic
targets that require better timing and prediction to hit successfully. Implementation
uses pygame with moving brick mechanics, oscillation patterns, collision detection,
...
class Brick:
    def __init__(self, x, y, color):
        self.center_x = x  # Original center position
        self.x = x
        self.y = y
        ...
        self.move_offset = 0  # Current movement offset from center
        self.move_direction = random.choice([-1, 1])  # Random initial direction

    def update(self):
        if not self.destroyed:
            # Move brick back and forth
            self.move_offset += self.move_direction * BRICK_MOVE_SPEED

            # Reverse direction when reaching movement limits
            if abs(self.move_offset) >= BRICK_MOVE_RANGE:
                self.move_direction *= -1

            # Update actual position
            self.x = self.center_x + self.move_offset

            # Keep brick on screen
            if self.x < 0:
                self.x = 0
                self.move_direction = 1
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Breakout - Moving Bricks Remix")
